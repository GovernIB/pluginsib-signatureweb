 ...
 *
 */
public abstract class AbstractSignatureWebPlugin extends
        AbstractWebPlugin<AbstractSignatureWebPlugin.SignIDAndIndex, SignaturesSetWeb> implements ISignatureWebPlugin {
    ...
    public StatusSignature getStatusSignature(String signaturesSetID, int signatureIndex) {
        ...
        try {
            ...
            log.warn("Error accedint a l'index " + signatureIndex + " del conjunt de firmes " + signaturesSetID);
            return null;
        }
    ...
    public void requestGET(String absolutePluginRequestPath, String relativePluginRequestPath, String query,
        ...
        if (query.startsWith(CANCEL_PAGE)) {
            ...
            super.requestGETPOST(absolutePluginRequestPath, relativePluginRequestPath,
                    new SignIDAndIndex(signaturesSet, signatureIndex), signaturesSet, query, languageUI, request,
                    response, true);
        }
    ...
    @Override
    public void requestGET(String absolutePluginRequestPath, String relativePluginRequestPath, String query,
            String signaturesSetID, int signatureIndex, HttpServletRequest request, HttpServletResponse response) {
    ...
    public void requestPOST(String absolutePluginRequestPath, String relativePluginRequestPath, String query,
        ...
        if (log.isDebugEnabled()) {
            logAllRequestInfo(request, "POST " + getSimpleName(), absolutePluginRequestPath, relativePluginRequestPath,
                    query, new SignIDAndIndex(signaturesSet, signatureIndex));
        }
    ...
    public void requestPOST(String absolutePluginRequestPath, String relativePluginRequestPath, String query,
        ...
        if (signaturesSet == null) {
            ...
            requestPOST(absolutePluginRequestPath, relativePluginRequestPath, query, signaturesSet, signatureIndex,
                    request, response, locale);
        }
    ...
    protected void cancel(HttpServletRequest request, HttpServletResponse response, SignaturesSetWeb signaturesSet) {
     ...
     */
    @Override
    public void finishWithError(HttpServletResponse response, SignaturesSetWeb signaturesSet, String errorMsg,
            Throwable th) {
        if (th == null) {
            log.error(errorMsg);
        } else {
    ...
    protected void errorPage(String errorMsg, Throwable th, String absolutePluginRequestPath,
            String relativePluginRequestPath, HttpServletRequest request, HttpServletResponse response,
        ...
        PrintWriter out = generateHeader(request, response, absolutePluginRequestPath, relativePluginRequestPath, lang,
                sai, signaturesSet);
        ...
        if (finalurl != null) {
            out.println("<td>");
            out.println("<button class=\"btn\" type=\"button\"  onclick=\"location.href='" + finalurl + "'\" >"
                    + getTraduccio(ABSTRACT_SIGNATURE_WEB_RES_BUNDLE, "tornar", locale) + "</button>");
            out.println("</td>");
            out.println("</tr>");
        }
    ...
    public static class SignIDAndIndex {
        ...
        public String toString() {
            return "(signatureID: " + signaturesSetID + " | " + "signatureIndex: " + signatureIndex + ")";
        }
    ...
    public void rubricPage(String relativePath, SignaturesSetWeb signaturesSet, int signatureIndex,
